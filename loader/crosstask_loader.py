...
class CrossTask_DataLoader(Dataset):
    """CrossTask Video-Text loader."""

    def __init__(
            self,
            data,
            video_root='./data/downstream/',
            num_clip=4,
            fps=16,
            num_frames=32,
            size=224,
    ...
    def __getitem__(self, idx):
        ...
        task = self.data['task_id'].values[idx]
        start = self.data['start'].values[idx]
        end = self.data['end'].values[idx]
        cap = self.data['text'].values[idx]
        if os.path.isfile(os.path.join(self.video_root+ '/crosstask/videos/'+ str(task) + '/' + video_id + '.mp4')):
            video_path = os.path.join(self.video_root+ '/crosstask/videos/'+ str(task) + '/' + video_id + '.mp4')
        else:
            raise ValueError
        text = self.words_to_ids(cap)
        video = self._get_video(video_path, start, end, self.num_clip)
        return {'video_id': video_id, 'task_id': task, 'video': video, 'text': text}